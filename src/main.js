...
if (filmCards.length !== 0) {
  ...
  filmCards.slice(MOCK_LILMS).forEach((film) => render(filmsListContainer, createCardFilm(film)));
...
if (filmCards.length > FILMS_CARDS_COUNT) {
  ...
  loadMoreButton.addEventListener(`click`, (evt) => {
    evt.preventDefault();
    filmCards.slice(renderedFilmCount, MOCK_LILMS).forEach((film) => render(filmsListContainer, createCardFilm(film)));
